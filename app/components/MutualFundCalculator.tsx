...
export default function MutualFundCalculator({ toolName }: MutualFundCalculatorProps) {
  ...
  const result = useMemo(() => {
    ...
    if (investmentType === 'sip') {
      ...
      return { futureValue, totalInvested, gains, yearlyData };
    } else {
      ...
      const yearlyData = [];
      for (let year = 1; year <= years; year++) {
        const yearValue = amount * Math.pow(1 + (annualReturn / 100), year);
        yearlyData.push({
          year,
          invested: amount,
          balance: yearValue,
          gains: yearValue - amount,
        });
      }

      return { futureValue, totalInvested: amount, gains, yearlyData };
    }
  }, [investmentType, amount, annualReturn, years]);

  return (
    <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">{toolName || 'Mutual Fund Return Calculator'}</h2>
